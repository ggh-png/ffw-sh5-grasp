...
전체(`app`)를 인자로 받아, HUD·양손 EE 포즈/관절각 슬라이더·시나리오별 grasp/squeeze
패널·리프트·관절 모니터까지 패널 전체를 매 프레임 그리고, 슬라이더/버튼 조작 결과를
`app`의 상태에 곧바로 되써넣는다.
...
패널 상단에는 `Cyclo Control`이 있다. ROBOTIS Cyclo Control 문서의 marker 기반
`MoveL`/`Bimanual MoveL` 흐름을 이 MuJoCo 텔레옵에 맞춘 영역이다. `MoveL`에서는
`right_goal_marker`/`left_goal_marker`가 각 손 목표이고, `Bimanual MoveL`에서
...
패널의 +/- 버튼은 같은 target을 작은 step으로 미세 조정하는 보조 입력이다. FK 모드인
손은 jog 대상에서 제외되고, box 모드에서 marker/gizmo를 쓰면 `_note_manual_pose_edit()`가
자동 정렬을 꺼서 수동 target이 다음 프레임에 덮이지 않는다.
...
3D 화살표/회전 링은 `teleop_app.py`의 `_draw_transform_gizmo()`가 ImGuizmo를 이용해
...
손별 패널은 IK/FK 모드에 따라 다른 슬라이더 집합을 그린다:
...
if mode == "ik":
    ...
    for i, axis in enumerate(("X", "Y", "Z")):
        changed, pos[i] = imgui.slider_float(f"{axis}##{side}pos", pos[i], -0.2, 1.2, "%.3f m")
        if changed:
            _note_manual_pose_edit(app)   # box auto-align이 수동 XYZ/RPY 입력을 덮지 않게 끔
    ...
else:
