...
| 구분 | 내용 |
|---|---|
| 해결할 문제 | 목표 손 위치와 자세를 관절 변화량으로 바꾸되, 특이점에서 관절 명령이 폭발하지 않고 자세 보정이 이미 맞춘 위치를 크게 훼손하지 않아야 한다. |
| 해결 방법 | 위치 Jacobian에 DLS를 적용하고, 자세 보정은 위치 task의 null-space projector를 통과시킨 뒤 관절 변화량과 관절 범위를 제한한다. |
| 사용 수식 | \(\Delta q_p=J_p^T(J_pJ_p^T+\lambda^2I)^{-1}e_p\), \(N_\lambda=I-J_p^T(J_pJ_p^T+\lambda^2I)^{-1}J_p\), \(\Delta q=\Delta q_p+N_\lambda J_R^Te_R\). 목적함수부터 이 식까지의 모든 등식은 [DLS와 위치 우선 IK 수학](../ik-math.md)에 생략 없이 전개했다. |
| 코드 구현 과정 | `solve_pose()`가 scratch state를 만들고 → `_jac()`과 `_pose_error()`를 계산하고 → `position_system`을 풀어 위치 step을 구하고 → projector를 적용한 자세 step을 더하고 → `_clamp_to_limits()`로 제한한다. |
| 수식 없이 사용하는 함수 | `_initialize_scratch()`와 `_write_q()`는 live state를 건드리지 않고 후보 관절각을 기록한다. `mujoco.mj_forward()`, `mujoco.mj_jacSite()`, `kinematics.shortest_orientation_error()`가 pose·Jacobian·자세 오차를 계산한다. `solve_pose_multistart()`는 여러 초기값 중 최선의 해를 고른다. |
