...
| MoveIt (IK 서비스, 모션 플래닝, 충돌 회피 경로) | `src/whole_body_ik.py`의 `WholeBodyIK` | base/lift/양팔 differential IK만 있고 ROS·경로 플래너는 없다 |
...
Cyclo 공식 구현에서 참고한 핵심은 `J^T W J` 형태의 weighted task와 velocity/joint
limit 구조다. 이 프로젝트는 Pinocchio/OSQP/ROS를 가져오지 않고 18변수 제한
least-squares를 NumPy active-set으로 직접 푼다.
...
| `test_whole_body.py` | ROS-free import gate, 스워브 왕복, 무작위 18DOF solver 40회, 실제 바퀴의 XYZ/yaw whole-body 추종 | headless |
...
| MoveIt IK/Servo | `whole_body_ik.WholeBodyIK` | base/lift/양팔 weighted differential IK; 플래너와 ROS 없음 |
