...
이건 흔히 "teleop 프로토타입"에서 쓰는 접근이고, 실제로 이 프로젝트도 `PLAN.md`
맨 위에 "사람이 조작해서 집는다(자율 실행 FSM 없음)"라고 명시하고 있다.
...
`PLAN.md` 맨 위 절대 규칙에 "kinematic override 금지"를 못 박고 시작한다 —
...
임의로 발명하지 않는다는 원칙, `PLAN.md` 참고).

## 3.2 절대 규칙 (모든 Phase에 적용, `PLAN.md`에서 그대로 인용)
...
이 다섯 규칙은 지금(Session 12)까지 한 번도 깨진 적이 없다 — Part 14의 버그
...
├── PLAN.md                    # 이 프로젝트의 "헌법"
└── NOTES.md                   # Phase별 튜닝 근거/버그 원인 상세 기록
```
...
잠겨 있다. 이건 튜닝 실패가 아니라 `PLAN.md`가 처음부터 제시한 fallback이다:
...
`mju_subQuat`로 계산한 방향 오차(`ori_err`)는 **site 로컬 프레임**인데, `jacr`
(회전 야코비안)은 **월드 프레임**이다. 이 둘을 그냥 곱하면 보정이 엉뚱한
관절로 샌다 — Phase 3에서 실제로 겪은 버그다. 고쳐진 코드:
...
```python
tau = qfrc_bias[joint]         # 지금 상태에서 중력+코리올리+원심력을 정확히 상쇄
    + kp * (q_des - q)          # 위치 피드백
    - kd * qvel                 # 속도 피드백(능동 댐핑)
```

핵심은 `qfrc_bias`다. MuJoCo가 매 스텝 계산해주는 이 값은 "지금 이 자세를
버티는 데 필요한 힘"(중력+코리올리+원심력)을 정확히 담고 있다. 이걸 토크에
더해주면 **kp를 아무리 올려도 못 없애던 정적 처짐이 사라진다** — kp를 올리는
문제가 아니라 애초에 안 하고 있던 피드포워드를 넣는 문제였다는 게 이 진단의
핵심 결론이다.
...
1. 몸체 속도 `(vx, vy, wz)`와 모듈 위치 `(module_x, module_y)`로 그 바퀴가
   내야 할 속도 벡터를 계산: `wheel_vel = (vx - wz*module_y, vy + wz*module_x)`.
2. 그 벡터의 각도가 목표 조향각, 크기가 목표 속력.
3. **180도 반전 최적화**: 목표 조향각이 현재 조향각과 90도 넘게 차이 나면,
   조향을 반대쪽으로 도는 대신 "조향각 + 180도"로 놓고 바퀴 회전 방향을
   반대로 뒤집는다 — 조향 모터가 최대 반바퀴만 돌면 되게 하는 최적화(실제
   스워브 로봇에서 흔히 쓰는 기법).
4. **정렬 게이팅**(alignment gating): 조향이 아직 목표각에 충분히 안 맞았으면
   (`align_err`가 임계값 이상) 그 바퀴의 구동 속도를 0으로 강제한다 — 조향이
   덜 된 상태에서 바퀴가 돌면 엉뚱한 방향으로 밀리기 때문.
5. **반전 시퀀스**(`ReversalPhase`): 바퀴 회전 방향이 바뀔 때는
...
`capture_grasp(app)`가 하는 일:

1. `sync_virtual_object_to_hand_targets` — virtual object를 지금 양손 target의
   중점으로 이동.
2. 그 순간 양손 각각의 world pose를, virtual object의 world pose 기준
   **상대 좌표**(`obj_R.T @ (hand_pos - obj_pos)`)로 저장(`cyclo_capture_offsets`).

이후 `apply_virtual_object_target(app)`가 매 프레임(가상 물체가 캡처된 동안)
그 상대 좌표를 virtual object의 **현재** pose에 다시 펼쳐서 양손 target을
갱신한다:
...
핵심 공식(회전) — "손 target의 world quaternion"은 세 개의 쿼터니언 곱이다:
...
Phase가 하나 끝날 때마다 `git tag phase-N`을 찍는다(`PLAN.md` 규칙 4).
