...
## 6.2 이 Part를 이해하는 데 필요한 수학: 벡터, 행렬, 야코비안, null space
...
## 6.3 문제 정의와 DLS(damped least squares)
...
## 6.4 계층형 IK — position이 orientation보다 우선
...
## 6.5 좌표계 함정 — site-local vs world
...
`mju_subQuat(ori_err, target_quat, cur_quat)`는 "지금 자세(`cur_quat`)에서
목표 자세(`target_quat`)까지 가는 데 필요한 회전"을 구해서, 그 회전을 "회전축
방향 × 회전각" 벡터 3개로 근사해 반환한다 — 각도가 작을수록 이 근사가 정확해진다
(6.2에서 본 "입력을 아주 조금 바꾸면"과 같은 1차 근사의 발상이다):

\[
e_{ori} \;\approx\; 2\,\mathrm{vec}\big(q_{target} \otimes q_{current}^{-1}\big)
\]

이 벡터(`ori_err`)는 **site 로컬 프레임**(그 회전이 "지금 site가 놓인 방향
기준"으로 표현돼 있다는 뜻)인데, `jacr`(회전 야코비안)은 **월드 프레임**으로
계산된다. 이 둘을 그냥 곱하면 보정이 엉뚱한 관절로 샌다 — Phase 3에서 실제로
겪은 버그다. 고쳐진 코드:
...
## 6.7 multistart — 국소해(local minimum) 탈출
...
## 6.8 "왜 관절이 갑자기 멈춘 것처럼 보일까" — stuck-frame 최적화
...
**왜 \(h(q,\dot q)\) 항이 있고 없고에 따라 결과가 이렇게 달라지는가** — 로봇 팔의
운동방정식은 일반적으로 \(M(q)\ddot q + h(q,\dot q) = \tau\) 형태다(\(M\)은
관성행렬, \(h\)는 중력+코리올리+원심력의 합, 바로 이게 `qfrc_bias`). 가만히
멈춰서 자세를 유지하는 정적 평형 상태에서는 \(\ddot q = \dot q = 0\)이므로,
그 자세를 버티는 데 **필요한 토크는 정확히 \(h(q,0)\)** 다(중력을 상쇄하는
힘, 팔이 무거울수록 크다).

- **feedforward 없이 순수 P 제어**(\(\tau = K_p(q_{des}-q)\))만 쓴다면, 평형
  조건 \(\tau = h(q,0)\)에서 \(K_p(q_{des}-q) = h(q,0)\), 즉 정상상태 오차는
  \(e = q_{des}-q = h(q,0)/K_p\)다 — **0이 아니고, \(K_p\)에 반비례할 뿐**이다.
  \(K_p\)를 5배로 올리면 오차가 1/5로 줄 뿐 절대 0이 되지는 않는다(무한대
  \(K_p\)가 아닌 한). 실제로 이 프로젝트에서 \(K_p\)를 5배로 올려봐도 처짐이
  거의 안 줄었던 게 바로 이 관계식대로다.
- **feedforward를 더하면**(\(\tau = h(q,\dot q) + K_p(q_{des}-q) - K_d\dot q\)),
  평형 조건이 \(h(q,0) = h(q,0) + K_p(q_{des}-q)\)가 되고, 양변에서
  \(h(q,0)\)이 상쇄되어 \(K_p(q_{des}-q)=0\) — \(K_p\)가 유한한 어떤 값이어도
  \(q=q_{des}\)에서 평형이 성립한다. **오차가 줄어드는 게 아니라 애초에 0이 되는
  평형점 자체가 바뀐다.**

이게 \(h(q,\dot q)\) 항의 핵심이다 — \(K_p\)를 올리는 문제가 아니라 애초에 안
하고 있던 피드포워드를 넣는 문제였다는 게 이 진단의 핵심 결론이다.
...
벡터식을 그대로 옮긴 것뿐이다(`+ 1e-9`는 \(v_{i,x}=v_{i,y}=0\)인 완전 정지
근처에서 `atan2`가 불안정한 값을 내는 걸 막는 안전장치):
...
1. **180도 반전 최적화**: 조향각 \(\theta_i\)와 \(\theta_i+180°\)는 "같은 직선
...
해결(바퀴 배치/마찰 재설계)은 다음 세션 과제로 남아 있다. 파라미터를 무작위로
더 세게 조정하는 대신 후보를 하나씩 순서대로 배제해 나간 이 접근(7.1의 진단
순서와 같은 원칙)이, 실전에서 "이 이상은 파라미터 문제가 아니다"라는 결론까지
어떻게 도달하는지 보여주는 좋은 사례다.
...
**왜 이 변환이 필요한가**: IK 솔버(`ik.py`)는 world 좌표계 기준 목표 pose를
받는다. 하지만 손 target 슬라이더는 "로봇 기준으로 앞/옆/위"를 조작하고
싶은 것이지, 베이스가 주행 중에 계속 움직이는 절대 world 좌표를 매번 다시
계산해서 넣고 싶은 게 아니다 — 베이스가 이동/회전하는 동안 target이 world
좌표에 고정돼 있으면, 팔이 "따라오는 베이스를 뒤쫓아 다시 뻗는" 것처럼 보여야
하는데 이는 실제 텔레옵 의도(손은 로봇에 붙어서 같이 움직이되, 그 위에서
상대적으로만 조작)와 다르다. 그래서 슬라이더가 표현하는 값은 항상
"베이스 기준 상대 위치/자세"이고, 그걸 IK가 원하는 world 좌표로 바꿔주는
변환이 매 프레임 필요하다. base-local 위치 \((x,y,z)\)를 world 위치로 옮기는
변환은 베이스의 현재 \((x_b, y_b, \theta_b)\)(위치 2개 + yaw 각) 만큼 2D 회전 후
