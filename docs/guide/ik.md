...
> [ROS2 개발자를 위한 가이드](ros2-guide.md) Part 6이 유도 과정을 포함해 자세히
> 다룬다. 여기서는 최종 식만 정리한다.
...
\(e_{ori}\)는 `mju_subQuat(target_quat, cur_quat)`로 구하며, 작은-각도 근사의
축각 벡터다: \(e_{ori} \approx 2\,\mathrm{vec}(q_{target}\otimes q_{current}^{-1})\).
이 값은 site 로컬 프레임이라, 월드 프레임인 \(J_r\)과 곱하기 전에
`site_xmat @ e_ori`로 먼저 월드 프레임으로 옮긴다.
