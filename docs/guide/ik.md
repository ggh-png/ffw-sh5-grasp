...
```mermaid
flowchart TD
    A["teleop_app._step_physics"] --> B["solve_pose(q_init, target_pos, target_quat)"]
    B --> C["scratch MjData reset"]
    C --> D["context_qpos copy"]
    D --> E["_write_q(q_init)"]
    E --> F["mj_forward()"]
    F --> G["_pose_error()"]
    G --> H["_jac()"]
    H --> I["_dls_step() for position"]
    H --> J["orientation null-space projection"]
    I --> K["combine / clamp dq"]
    J --> K
    K --> L["backtracking line search"]
    L --> M["_write_q(q_try)"]
    M --> F
    L --> N["return q_solution, errors"]

    O["solve_pose_multistart()"] --> B
    O --> P["retry random joint seeds"]
```
