...
## Phase 5 후속 10 — 실제 STL 메시에 라벨을 직접 감기 (2026-07-05)

**커밋:** `46f773c`

### 사용자 지적: 대체 원통 말고 진짜 `soda_can.stl`을 쓰라는 요청

바로 앞 절(후속 9)에서 라벨을 감는 데는 성공했지만, 실제 방식은 **디테일한
STL 메시를 파라메트릭 원통으로 통째로 대체**한 것이었다(림/풀탭/돔형 상단/오목한
바닥 형상이 전부 사라지고 밋밋한 원통+평평한 캡만 남음). 사용자가 명확히
`assets/soda_can/soda_can.stl`을 쓰고 여기에 `soda_can_label.png`를 감으라고
재지적 — 진짜 메시 형상을 유지한 채 텍스처만 입혀야 하는 과제로 전환.

### 접근: 얼굴 방향이 아니라 높이(z) 프로파일로 옆면/캡 분리

STL은 UV가 없으므로(후속 9와 동일한 제약) 여전히 UV를 직접 계산해서 얹어야 하는데,
이번엔 **실제 메시의 버텍스**에 대해 계산해야 함. 먼저 "면 법선이 수평에 가까우면
옆면, 수직에 가까우면 캡"이라는 직관적 기준을 시도했으나, 법선 z성분 절대값
임계치 0.3만 잡아도 **전체 면의 78%가 "캡"으로 분류**되는 이상 현상 발견 —
풀탭/림 디테일이 국소적으로 매우 가파른 형상이라 "시각적으로는 옆면인 영역"도
법선 기준으로는 캡처럼 보이는 것. 대신 **버텍스의 반지름을 높이(z)의 함수로
직접 프로파일링**: z 방향을 40구간으로 나눠 각 구간의 반지름 평균/표준편차를
찍어보니 **z 전체 범위의 약 5%~92% 구간은 반지름이 완전히 일정(표준편차 0)한
순수 원통**이고, 나머지 상단 8%(돔+림+탭, 버텍스 약 1만 개 집중)와 하단 5%(오목
바닥)에 디테일이 전부 몰려있음을 확인 — 기하학적으로 이미 "라벨이 붙는 영역"이
어디인지 메시 자체가 알려주고 있었던 것. 이 z-비율 기준으로 면을 옆면/캡 두
그룹(1024개 vs 28878개, 전체 29902개 중)으로 분리.

### 비정형 삼각분할의 이음매 처리

수작업으로 만든 정규 격자(후속 9의 원통)와 달리 이번엔 STL의 **비정형
삼각분할**이라 "이음매에 있는 특정 버텍스들"을 미리 알고 중복시켜두는 방식이
안 통함. 대신 **face(삼각형)마다 독자적으로 UV를 계산**: 각 삼각형의 3개 꼭짓점
각도(atan2)를 구하고, 첫 번째 꼭짓점 기준으로 나머지 두 개가 ±π를 넘어가면
2π를 더하거나 빼서 "펼친" 뒤 u로 변환 — vt를 정점별로 공유하지 않고 매 face마다
3개씩 새로 발급(OBJ가 `f v/vt` 형태로 위치 인덱스와 텍스처 인덱스를 독립적으로
지정할 수 있다는 점 활용)하는 방식으로, 어떤 삼각형이 이음매를 걸치고 있어도
항상 올바르게 처리됨.

### 결과물

`tests/generate_can_label_mesh.py`를 완전히 재작성(이전 버전은 순수 파라메트릭
원통 생성기였음) — `assets/soda_can/soda_can.stl`을 trimesh로 읽어 위 로직으로
`can_side_detail.obj`(라벨 텍스처, 1024면)와 `can_cap_detail.obj`(무광 은색,
28878면)를 생성. 둘 다 원본 STL의 버텍스 좌표를 그대로 쓰므로(면만 나눔) 이어
붙였을 때 gap도 seam도 없음. 8방향 스윕 + 풀탭 클로즈업 렌더로 확인: 로고/버블/
영양정보 패널이 라벨 영역에 또렷하게 감기고, 캡 영역엔 실제 풀탭·림·돔 형상이
살아있는 채로 무광 은색이 자연스럽게 입혀짐. 이전 세션(후속 9)의 파라메트릭
원통 대체물(`can_side.obj`)과 그 절단면 2개(top/bottom cap 원판)는 완전히 폐기.

### 검증

`tests/test_phase_{0,1,2,3,4,5}.py` 전부 PASS, 수치 전부 이전과 완전히 동일(pick
8/10, idle drift 0.0002mm 등) — 이번에도 순수 시각 변경, `can_geom`(실제 충돌
콜리전)은 전혀 안 건드림. `python3 src/teleop_app.py` 15초 스모크 클린 종료.

### 교훈

**"이 정도면 됐다"고 넘어가기 전에 사용자가 실제로 요청한 것과 내가 만든 것
사이의 차이를 다시 확인해야 한다** — 후속 9는 "텍스처가 감긴다"는 기술적 목표는
달성했지만, 사용자가 명시한 특정 파일(`soda_can.stl`)을 실제로 쓰지는 않고
우회했다(문제를 더 쉬운 형태로 바꿔서 풀었을 뿐). 사용자가 정확한 경로 두 개를
다시 짚어준 것 자체가 "그 우회가 요구사항을 충족 못 했다"는 신호였다. 또한
**"면 법선"이라는 그럴듯한 기하학적 기준도 검증 없이 채택하면 안 된다** — 78%가
캡으로 분류되는 이상한 결과가 나온 순간 바로 그 기준을 버리고 더 직접적인
지표(반지름-높이 프로파일)로 바꾼 것이 시간을 아꼈다.

---

## Session 11 — 주행 릴리즈 역주행 버그 수정 + 회전 개선 + 스트레이프 키 변경 (2026-07-08)

### 사용자 지적: "방향키를 놓는 순간 바퀴가 다시 뒤로 밀림" + "회전이 바퀴로 안 굴러가는 느낌"

오프라인 물리 서브스텝 로깅(`data.qvel`/`data.contact` 직접 관찰, GUI 없이)으로
두 증상을 독립적으로 재현·진단.

### 버그 1 — 키를 놓으면 속도가 감쇠가 아니라 부호까지 뒤집힘

`base_teleop.BaseTeleop.update`의 감쇠 자체(`v_local *= exp(-K_BRAKE*dt)`)는
수학적으로 0으로만 수렴하고 절대 음수로 못 넘어간다 — 그런데 실측 물리 속도는
정반대였다: "s"(후진)로 정상상태 -0.49m/s에 도달한 뒤 키를 떼면, 단 한 프레임
(40ms) 안에 **+0.63m/s까지 정반대로 튐**(원래 방향으로는 아예 안 멈추고 반대
방향으로 원래 속도보다 더 크게 넘어감). 접촉력 직접 로깅으로 원인 특정: 릴리즈
순간 바퀴 3개 전부에 순간 ~1877N(x축)의 큰 반력이 걸리고 지수적으로 감쇠 —
`wheel_drive`의 `kv=150`(Phase 5 후속 8에서 회전 토크 확보용으로 올린 값)이
바퀴 자체의 회전관성(대각관성 ~0.01)에 비해 지나치게 강한 속도 서보라, 매 프레임
한 번씩(`BaseTeleop.update`가 프레임당 1회 호출) 목표가 계단식으로 바뀔 때마다
그 스텝 크기만큼 강한 토크를 걸고, 이게 고마찰(μ=5.0) 바퀴-바닥 접촉을 통해
100kg 베이스 전체를 순간적으로 "잡아채는" 효과를 냄 — **감쇠/가속 자체의 문제가
아니라 시간상수(K_ACCEL=3.0/K_BRAKE=6.0/YAW_FOLLOW=4.0/YAW_DECAY=10.0)가 이
바퀴-접촉 강성에 비해 너무 빨라서, 목표가 한 프레임 안에 너무 크게 움직인 것**.
직접 검증: `drive.update`를 프레임당 1회가 아니라 물리 서브스텝(1ms)마다 호출해도
동일한 튐이 재현됨 → "계단식 업데이트"가 원인이 아니라 순수 시간상수 크기가
원인이라는 뜻. K_ACCEL/K_BRAKE/YAW_FOLLOW/YAW_DECAY를 {6, 3, 1.5, 1.2, 1, 0.8,
0.6}으로 스윕해 "역주행이 완전히 사라지는(overshoot<=0) 가장 빠른 값"을 찾음 —
번역/회전 축 모두 **1.0**(원래 값 대비 3~10배 완화)에서 역전 없이 약 1초 내 정지,
정상상태 속도/회전비는 전혀 안 바뀜(직접 재검증). `kv`/조향 `kp`/접촉 `solref`는
전혀 안 건드림 — 이 값들을 바꾸면 아래 회전 개선과 상충(다음 절 참고).

### 버그 2로 짚었던 것의 실체 — 회전은 "안 굴러가는" 게 아니라 "미끄러지는" 것

전륜(좌/우) 조향각을 실측하니 명령(±28.2°)까지 전혀 못 가고 항상 **±18.5° 부근에
멈춤** — 반면 후륜은 명령(-90°)에 거의 정확히 도달. 이 언더슈트가 실제 원인인지
가설을 순서대로 기각: (1) 조향 `kp`를 5000→50000(10배)까지 올려도 정지각이 거의
안 움직임(-18.5°→-20.1°, 선형 P-제어 드룹이라면 10배 줄어야 함) → 기각.
(2) 조향 `forcerange`를 2000→8000으로 늘려도 실측 토크 자체가 애초에 2000 근처도
안 감(unsaturated) → 기각. (3) 구동 `kv`를 4~150까지 스윕해도 정지각은 고정
(-18.6~-18.7°) → 구동 토크와 무관 → 기각. 유일하게 반응한 변수: 바퀴-바닥
`<pair>` solref의 timeconst(0.15→0.7, 더 무르게)를 올리면 조향각은 그대로인데
회전비가 22.6%→31.0%까지 꾸준히 개선(0.8 이상에서 갑자기 불안정하게 붕괴,
6.7%로 폭락 — 절벽 근처). 즉 이건 게인 튜닝으로 푸는 문제가 아니라 **바퀴가
목표 각도로 못 돌고 미끄러지는 상태에서 마찰이 감당할 수 있는 순수 견인력
한계**(Phase 5 후속 8에서 이미 "더 근본적인 한계"로 짚었던 바로 그것)라는
직접 증거. solref를 더 무르게 해서 회전을 개선하면 버그 1(역주행)이 **똑같은
파라미터를 통해** 재발함을 확인(solref=0.5 조합에서 release_overshoot가 다시
+0.63 근처로 돌아옴, drive slip도 19.5%로 한도(15%) 초과) — 두 증상이 물리적으로
결합돼 있어 solref 쪽으로는 더 안 건드리기로 함.

### 채택한 타협: K_YAW를 게이트가 아니라 목표 자체를 올림

회전비(achieved/commanded)가 K_YAW=1.2→2.0 구간에서는 거의 그대로(22.6%→22.9%)
유지됨을 확인(3.0~8.0에서는 오히려 더 나빠지고 12.0에서는 다른 영역으로 튀는 등
비단조적이라 더 올리는 건 보류) — 같은 여유폭에서 목표만 올려 달성 각속도를
15.6°/s→26.2°/s(약 1.67배)로 개선. `kv`/조향 `kp`/`forcerange`/`solref`는 전부
원래값 유지(회귀 위험 없음).

### 스트레이프 키 변경

사용자 요청대로 `Shift+Left/Right` → `[`/`]`로 이동(Left/Right는 순수 yaw
전용으로 남김, 모디파이어 없이 별도 키). `src/teleop_app.py`(`_read_drive_and_lift_keys`),
`src/teleop_ui.py`(HUD), `README.md`, `docs/run.md` 전부 갱신.

### 검증

`tests/test_phase_5.py` 전부 PASS(idle drift 0.0002mm 동일, 직진 slip 3.9%,
역주행 overshoot -0.0002 — 사실상 0, 충돌 정지 447mm 동일). `tests/test_phase_4.py`
pick 8/10(변경 전과 동일 — 바퀴/시간상수 변경은 손 grasp과 무관). 오프라인
회전/스트레이프 릴리즈 트레이스로 두 축 모두 역전 없음 재확인. `python3
src/teleop_app.py` 8초 스모크 클린 종료(에러 없음).

### 알려진 한계 (계속 이월)

전륜이 명령 조향각에 못 미치는 근본 원인(순수 마찰/견인력 한계로 추정되나
정확한 역학적 메커니즘은 미확정)은 이번에도 완전히 못 풀었다 — 게인
조합(조향 kp, 구동 kv, forcerange)을 전부 독립적으로 스윕해 전부 기각했다는
점에서 Phase 5 후속 8보다 원인을 더 좁혔지만("바퀴 배치/마찰계수 자체의
재설계가 필요"쪽에 더 가까워짐), 실제 재설계는 이번 세션 범위 밖. 스트레이프도
동일한 저조향각 한계를 공유(정상상태 속도가 명령 0.5m/s의 ~19%에 그침, 이번
세션에선 손 안 댐 — 사용자가 요청한 건 키 재배치뿐이었음).
